...
import Coo from './coo';
import { source } from './A'; // TODO : harmoniser parsing avec classe ProgressiveCat
...
var Catalog = function () {
  ...
  function findRADecFields(fields, raField, decField) {
    ...
    if (decField) {
      ...
      for (var _l = 0, _len = fields.length; _l < _len; _l++) {
        var _field = fields[_l];
        ...
        if (_field.ID && _field.ID === decField || _field.name && _field.name === decField) {
          decFieldIdx = _l;
          break;
        }
      }
    ...
    for (var _l2 = 0, _len2 = fields.length; _l2 < _len2; _l2++) {
      if (raFieldIdx != null && decFieldIdx != null) {
        break;
      }

      var _field2 = fields[_l2];

      if (!raFieldIdx) {
        if (_field2.ucd) {
          var ucd = _field2.ucd.toLowerCase().trim();
      ...
      if (!decFieldIdx) {
        if (_field2.ucd) {
          var _ucd = _field2.ucd.toLowerCase().trim();

          if (_ucd.indexOf('pos.eq.dec') === 0 || _ucd.indexOf('pos_eq_dec') === 0) {
            decFieldIdx = _l2;
            continue;
          }
        }
    ...
    if (raFieldIdx == null && decFieldIdx == null) {
      for (var _l3 = 0, _len3 = fields.length; _l3 < _len3; _l3++) {
        var _field3 = fields[_l3];
        var name = _field3.name || _field3.ID || '';
  ...
  Catalog.prototype.addSourcesAsArray = function (columnNames, array) {
    ...
    for (var rowIdx = 0; rowIdx < array.length; rowIdx++) {
      ...
      for (var _colIdx = 0; _colIdx < columnNames.length; _colIdx++) {
        dataDict[columnNames[_colIdx]] = row[_colIdx];
      }
  ...
  Catalog.prototype.draw = function (ctx, projection, frame, width, height, largestDim, zoomFactor) {
    ...
    for (var _k = 0, _len4 = sourcesInView.length; _k < _len4; _k++) {
      source = sourcesInView[_k];
    ...
    if (this.displayLabel) {
      ...
      for (var _k2 = 0, _len5 = sourcesInView.length; _k2 < _len5; _k2++) {
        Catalog.drawSourceLabel(this, sourcesInView[_k2], ctx);
      }
    }
  };
  ...
  Catalog.drawSource = function (catalogInstance, s, ctx, projection, frame, width, height, largestDim, zoomFactor) {
    ...
    var sourceSize = catalogInstance.sourceSize; // TODO : we could factorize this code with Aladin.world2pix
