  ...
  SimbadPointer.query = function (ra, dec, radiusDegrees, aladinInstance) {
    ...
    var successCallback = function successCallback(result) {
      ...
      if (match) {
        ...
        link.setAttribute("href", "http://simbad.u-strasbg.fr/simbad/sim-id?Ident=".concat(encodeURIComponent(objName)));
        link.setAttribute("target", "_blank");
        title.appendChild(link);
        var content = document.createElement("div");
        content.classList.add("aladin-sp-content"); // var content = '<div class="aladin-sp-content">';

        var em = document.createElement("em");
        em.textContent = "Type: ".concat(match[4]);
        em.appendChild(document.createElement("br"));
        content.appendChild(em);
        var magnitude = match[3];

        if (Utils.isNumber(magnitude)) {
          var magContent = document.createElement("em");
          magContent.textContnt = "Mag: $magnitude";
          magContent.appendChild(document.createElement("br"));
          content.appendChild(magContent);
        }

        var cdsLink = document.createElement("a");
        cdsLink.setAttribute("href", "http://cdsportal.u-strasbg.fr/?target=".concat(encodeURIComponent(objName)));
