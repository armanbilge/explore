...
var Overlay = function () {
  ...
  Overlay.parseSTCS = function (stcs) {
    ...
    while (k < len) {
      ...
      if (s === 'polygon') {
        ...
        var _frame = parts[k].toLowerCase();

        if (_frame === 'icrs' || _frame === 'j2000' || _frame === 'fk5') {
          while (k + 2 < len) {
        ...
        if (frame === 'icrs' || frame === 'j2000' || frame === 'fk5') {
          var _ra = parseFloat(parts[k + 1]);

          var _dec = parseFloat(parts[k + 2]);

          var radiusDegrees = parseFloat(parts[k + 3]);
          footprints.push(circle(_ra, _dec, radiusDegrees));
          k += 3;
        }
      }
  ...
  Overlay.prototype.draw = function (ctx, projection, frame, width, height, largestDim, zoomFactor) {
    ...
    for (var _k = 0, _len = this.overlays.length; _k < _len; _k++) {
      if (!this.overlays[_k].isSelected) {
        continue;
      }

      this.drawFootprintSelected(ctx, xyviews[_k]);
    }

    ctx.stroke(); // 2. Circle and polylines drawing

    for (var _k2 = 0; _k2 < this.overlay_items.length; _k2++) {
      this.overlay_items[_k2].draw(ctx, projection, frame, width, height, largestDim, zoomFactor);
    }
  };
  ...
  Overlay.prototype.drawFootprint = function (f, ctx, projection, frame, width, height, largestDim, zoomFactor) {
    ...
    if (show) {
      ...
      for (var _k3 = 1, _len2 = xyviewArray.length; _k3 < _len2; _k3++) {
        ctx.lineTo(xyviewArray[_k3].vx, xyviewArray[_k3].vy);
      }
    } else {//return null;
    } // end for
