...
var View = function () {
  ...
  View.prototype.setZoomLevel = function (level) {
    ...
    this.zoomFactor = this.computeZoomFactor(this.zoomLevel); // var oldFov = this.fov;

    this.fov = this.computeFov(this); // TODO: event/listener should be better

    this.updateFovDiv(this);
    this.computeNorder();
    this.forceRedraw();
    this.requestRedraw(); // on avertit les catalogues progressifs
  ...
  var unknownSurveyId = undefined; // @param imageSurvey : HpxImageSurvey object or image survey identifier
  ...
  View.prototype.setImageSurvey = function (imageSurvey, callback) {
    ...
    if (newImageSurvey && this.imageSurvey && Object.prototype.hasOwnProperty.call(newImageSurvey, 'id') && Object.prototype.hasOwnProperty.call(imageSurvey, 'id') && newImageSurvey.id === this.imageSurvey.id) {// do nothing
    } else {
      // buffer reset
  ...
  View.prototype.changeFrame = function (cooFrame) {
    ...
    this.cooFrame = cooFrame; // recompute viewCenter
      ...
      for (var k = 0; k < this.overlays.length; k++) {
        ...
        for (var i = 0; i < overlay.overlays.length; i++) {
          ...
          for (var j = 0; j < footprint.polygons.length; j++) {
            var xy = AladinUtils.radecToViewXy(footprint.polygons[j][0], footprint.polygons[j][1], this.projection, this.cooFrame, this.width, this.height, this.largestDim, this.zoomFactor);
        ...
        for (var _i3 = 0; _i3 < overlay.overlay_items.length; _i3++) {
          if (overlay.overlay_items[_i3] instanceof Circle) {
            overlay.overlay_items[_i3].draw(ctx, this.projection, this.cooFrame, this.width, this.height, this.largestDim, this.zoomFactor, true);

            if (ctx.isPointInStroke(x, y)) {
              var _closest2 = overlay.overlay_items[_i3];
              return [_closest2];
            }
          }
        }
