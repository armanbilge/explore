...
export function getSignText(num) {
    return num > 0 ? '+' : '';
}
export function getDiffSizeText(size) {
    return `${getSignText(size)}${bytes(size)}`;
}
export function getDiffPercentText(percent) {
    if (Number.isFinite(percent)) {
        return `${getSignText(percent)}${percent}%`;
    }
    return '-';
}
export function getLimitsCellText(file) {
    const limits = [];
    if (file.maxSize) {
...
export function getReportConclusionText(report) {
    ...
    if (fileFails === null || fileFails === void 0 ? void 0 : fileFails[0]) {
        return getReportConclusionTextForSingleFail(fileFails[0], 'file');
    }
    else if (groupFails === null || groupFails === void 0 ? void 0 : groupFails[0]) {
...
function getReportConclusionTextForSingleFail(singleFail, type) {
    ...
    if (singleFail.failReasons.length > 1) {
        ...
        if (singleFail.maxSize && singleFail.failReasons[0] === FailReason.MaxSize) {
            return `Max size exceeded in ${type} "${singleFail.path}": ${bytes(singleFail.size)} > ${bytes(singleFail.maxSize)}`;
        }
