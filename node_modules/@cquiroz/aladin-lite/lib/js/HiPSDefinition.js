...
var HiPSDefinition = function () {
  // constructor
  var HiPSDefinition = function HiPSDefinition(properties) {
  ...
  HiPSDefinition.prototype = {
    getServiceURLs: function getServiceURLs() {// TODO: TO BE COMPLETED
    },
  ...
  var merge = function merge(baseList, newList) {
    ...
    for (var _k = 0; _k < baseList.length; _k++) {
      var _item = baseList[_k];
      var id = _item.ID;

      if (Object.prototype.hasOwnProperty.call(newListById, id)) {
        var itemToAdd = newListById[id]; // we keep the last used URL property

        if (Object.prototype.hasOwnProperty.call(_item, LAST_URL_KEY) && !Object.prototype.hasOwnProperty.call(itemToAdd, LAST_URL_KEY)) {
          itemToAdd[LAST_URL_KEY] = _item[LAST_URL_KEY];
        }

        updatedList.push(itemToAdd);
      } else {
        updatedList.push(_item);
      }
    }
  ...
  HiPSDefinition.init = function () {
    ...
    for (var _k2 = indicesToRemove.length - 1; _k2 >= 0; _k2--) {
      localDefs.splice(indicesToRemove[_k2], 1);
    } // 2.2 merge
  ...
  HiPSDefinition.parseHiPSProperties = function (propertiesStr) {
    ...
    for (var k = 0; k < lines.length; k++) {
      var l = lines[k].trim(); // ignore comments lines
  ...
  HiPSDefinition.fromURL = function (url, callback) {
    ...
    Utils.getAjaxObject(propertiesUrl, 'GET', 'text', false).then(res => {
      var type = res.headers.get('content-type');
      if (type.includes("application/json")) return res.json();else if (type.includes("text/html")) return res.text();
    }).then(data => callbackWhenPropertiesLoaded(data)).catch(() => {
      Utils.getAjaxObject(propertiesUrl, 'GET', 'text', true).then(res => {
        var type = res.headers.get('content-type');
        if (type.includes("application/json")) return res.json();else if (type.includes("text/html")) return res.text();
      }).then(data => callbackWhenPropertiesLoaded(data)).catch(() => typeof callback === 'function' && callback(null));
    });
